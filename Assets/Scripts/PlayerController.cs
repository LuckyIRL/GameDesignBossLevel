...
public class PlayerController : MonoBehaviour
{
    ...
    public void Interact(InputAction.CallbackContext context)
    {
        if (context.started)
        {
            ...
            {
                InteractionScript3D interactionScript = hit.collider.GetComponent<InteractionScript3D>();
                if (interactionScript != null)
                {
                    interactionScript.interacted.Invoke();
                }
            }
        }
    }
    ...
    public void Quit(InputAction.CallbackContext context)
    {
        if (context.started)
        {
            // Check if the player is currently in the cannon
            CannonShooting cannon = GetComponent<CannonShooting>();
            if (cannon != null && cannon.cannonActive) // Remove () from cannonActive
            {
                // Exit the cannon
                cannon.ExitCannon();
            }
            ...
            {
                // Quit the game if not in the cannon
                Application.Quit();
            }
        }
    }
